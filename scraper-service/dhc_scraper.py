...
class DHCJudgmentScraper:
    def __init__(self, mongo_uri: str = "mongodb://admin:password@localhost:27018/indian_law_db?authSource=admin"):
        """Initialize the Delhi High Court Judgment Scraper"""
        self.base_url = "https://delhihighcourt.nic.in"
        self.judgments_url = "https://delhihighcourt.nic.in/web/"
        ...
        try:
            ...
            logger.info("MongoDB connection established for DHC scraper")
        except Exception as e:
            logger.error(f"MongoDB connection failed: {e}")
            raise

    def get_judgment_list(self, limit: int = 5) -> List[Dict]:
        """Fetch judgment list from Delhi High Court website"""
        try:
            ...
            # Look for judgment links in the latest judgments section
            # Based on the website structure from search results
            judgment_links = soup.find_all('a', href=re.compile(r'judgments?|judgment'))
            
            # Look for links in the "Latest Judgment" section
            latest_section = soup.find('div', string=re.compile(r'Latest Judgment', re.I))
            if latest_section:
                parent = latest_section.find_parent()
                if parent:
                    judgment_links.extend(parent.find_all('a', href=True))
            
            # Also look for links in the "Latest Pronouncement" section
            pronouncement_section = soup.find('div', string=re.compile(r'Latest Pronouncement', re.I))
            if pronouncement_section:
                parent = pronouncement_section.find_parent()
                if parent:
                    judgment_links.extend(parent.find_all('a', href=True))
            
            # Look for links with judgment patterns in the text
            judgment_text_links = soup.find_all('a', string=re.compile(r'BAIL|CRL|WP|CIVIL|CRIMINAL', re.I))
            judgment_links.extend(judgment_text_links)
    ...
    def extract_title(self, soup: BeautifulSoup, link_element=None) -> str:
        ...
        return "Delhi High Court Judgment"
    ...
    def extract_judges(self, soup: BeautifulSoup) -> List[str]:
        ...
        return list(set(judges)) if judges else ["Delhi High Court"]
    ...
    def generate_judgment_id(self, title: str, date: str) -> str:
        ...
        return f"dhc_{id_suffix}_{clean_date}"
    ...
    def scrape_judgments(self, limit: int = 5) -> Dict:
        ...
        try:
            logger.info(f"Starting Delhi High Court judgment scraping (limit: {limit})")
